...
class GuestsList {
    ...
    public int add(Guest g) {
        if (!isOnTheListAlready(g)){
            if(numberOfAvailableSpots() > 0){
                guestList.add(g);
                System.out.println("[" + g.fullName() + "] Congrats! Your place at the event is confirmed!");
                return 0;
            }else{
                waitingList.add(g);
                System.out.println("[" + g.fullName() + "] You successfully signed up to the guest list, on position " + waitingList.size() +
                        ". We will inform you should a spot become available.");
    ...
    public boolean remove(String firstName, String lastName) {
        ...
        if (toRemove != null) {
            ...
            if((guestList.size() < availableSeats) && (waitingList.size()!=0)) {
                ...
                System.out.println("Congrats, " + guestList.get(guestList.size()-1).fullName() + "! Your place at the event is confirmed!");
            }
    ...
    public String toString() {
        ...
        for (int i = 0; i < guestList.size() - 1; i++){
            displayGuests +=((i+1) + ". Name: " + guestList.get(i).getFirstName() + " " + guestList.get(i).getLastName() +
                    ", Email: " + guestList.get(i).getEmail() +
                    ", Phone: " + guestList.get(i).getPhoneNumber() + "\n");
        }
